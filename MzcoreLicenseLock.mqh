...
extern string License_JSON_URL        = "https://raw.githubusercontent.com/Zikri123123/core2026SNR-lock/main/licenses.json";
...
bool IsExpired(const string chunk)
{
   ...
   StringTrimLeft(expDate);
   StringTrimRight(expDate);
...
bool FindAccountChunk(const string response, const int account, string &chunkOut)
{
   ...
   int endObj = StringFind(response, "}", pos);
   if(endObj < 0) endObj = pos + 200;
   chunkOut = StringSubstr(response, pos, endObj - pos + 1);
   return true;
}
...
{
   string chunk = "";
   if(!FindAccountChunk(response, account, chunk))
   {
